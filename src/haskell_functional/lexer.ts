...
export function lexHaskellFunctionalSource(source: string): readonly HaskellFunctionalToken[] {
  ...
  const tokens: HaskellFunctionalToken[] = [];
  ...
  while (offset < source.length) {
    ...
    if (source.startsWith("{-", offset)) {
      ...
      if (end < 0) {
        throw new HaskellFunctionalSyntaxError(
          byteOffsets.span(commentStart, source.length),
          `Haskell block comment at byte ${
            byteOffsets.span(commentStart, commentStart).startByte
          } is unterminated.`,
        );
      }
      offset = end + 2;
    ...
    throw new HaskellFunctionalSyntaxError(
      span,
      `Haskell functional profile does not recognize ${
...
function isIdentifierContinue(codeUnit: number): boolean {
  return isIdentifierStart(codeUnit) || isDigit(codeUnit) || codeUnit === 0x5f || codeUnit === 0x27;
}
