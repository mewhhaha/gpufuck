...
import { lowerOcamlFunctionalSource, renderOcamlFunctionalTrace } from "./ocaml_functional.ts";

type OcamlFunctionalCommand = "run" | "trace";
...
export async function main(
  ...
  if (!isCommand(command) || sourcePath === undefined || arguments_.length > 3) {
    output.error("usage: ocaml_functional_cli.ts run <source.ml>");
    output.error("       ocaml_functional_cli.ts trace <source.ml> [trace.md]");
    return 2;
  }
  ...
  try {
    ...
    try {
      ...
      const trace = renderOcamlFunctionalTrace({
        title: `OCaml functional compilation trace: ${fileName(sourcePath)}`,
        source,
...
async function readSource(path: string): Promise<string> {
  try {
    ...
    throw new Error(`could not read OCaml functional source ${JSON.stringify(path)}${reason}`, {
      cause,
    });
  }
}
...
async function writeTrace(path: string, trace: string): Promise<void> {
  try {
    ...
    const reason = cause instanceof Error ? `: ${cause.message}` : "";
    throw new Error(`could not write OCaml functional trace ${JSON.stringify(path)}${reason}`, {
      cause,
    });
  }
}
...
function isCommand(value: string | undefined): value is OcamlFunctionalCommand {
