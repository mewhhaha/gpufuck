...
import { lowerRustFunctionalSource, renderRustFunctionalTrace } from "./rust_functional.ts";

type RustFunctionalCommand = "run" | "trace";
...
export async function main(
  ...
  if (!isCommand(command) || sourcePath === undefined || arguments_.length > 3) {
    output.error("usage: rust_functional_cli.ts run <source.rs>");
    output.error("       rust_functional_cli.ts trace <source.rs> [trace.md]");
    return 2;
  }
  ...
  try {
    ...
    try {
      ...
      const trace = renderRustFunctionalTrace({
        title: `Rust functional compilation trace: ${fileName(sourcePath)}`,
        source,
...
async function readSource(path: string): Promise<string> {
  try {
    ...
    throw new Error(`could not read Rust functional source ${JSON.stringify(path)}${reason}`, {
      cause,
    });
  }
}
...
async function writeTrace(path: string, trace: string): Promise<void> {
  try {
    ...
    const reason = cause instanceof Error ? `: ${cause.message}` : "";
    throw new Error(`could not write Rust functional trace ${JSON.stringify(path)}${reason}`, {
      cause,
    });
  }
}
...
function isCommand(value: string | undefined): value is RustFunctionalCommand {
