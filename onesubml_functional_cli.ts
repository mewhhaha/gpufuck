...
type OneSubmlFunctionalCommand = "run" | "trace";
...
export async function main(
  ...
  if (!isCommand(command) || sourcePath === undefined || arguments_.length > 3) {
    output.error("usage: onesubml_functional_cli.ts run <source.ml>");
    output.error("       onesubml_functional_cli.ts trace <source.ml> [trace.md]");
    return 2;
  }
  ...
  try {
    ...
    try {
      ...
      const trace = renderOneSubmlFunctionalTrace({
        title: `1SubML functional compilation trace: ${fileName(sourcePath)}`,
        source,
...
async function readSource(path: string): Promise<string> {
  try {
    ...
    throw new Error(`could not read 1SubML functional source ${JSON.stringify(path)}${reason}`, {
      cause,
    });
  }
}
...
async function writeTrace(path: string, trace: string): Promise<void> {
  try {
    ...
    const reason = cause instanceof Error ? `: ${cause.message}` : "";
    throw new Error(`could not write 1SubML functional trace ${JSON.stringify(path)}${reason}`, {
      cause,
    });
  }
}
...
function isCommand(value: string | undefined): value is OneSubmlFunctionalCommand {
